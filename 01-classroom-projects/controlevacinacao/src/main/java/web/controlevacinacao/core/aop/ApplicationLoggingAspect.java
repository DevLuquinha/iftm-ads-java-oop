package web.controlevacinacao.core.aop;
...
public class ApplicationLoggingAspect {
	...
	private void logMethodName(ProceedingJoinPoint joinPoint) {
		...
		String nomeClasseApenas =
				(posicaoPonto != -1) ? nomeCompletoClasse.substring(posicaoPonto + 1)
						: nomeCompletoClasse;
		logger.trace("Entrou no método: {}.{}", nomeClasseApenas,
	...
	private void logParametersReceived(ProceedingJoinPoint joinPoint) {
		...
		if (!parametros.isEmpty()) {
			...
			for (String nomeParametro : parametros.keySet()) {
				// Don't log the model, the BindingResult and the RedirectAttributes
				parametro = parametros.get(nomeParametro);
				if (!(parametro instanceof org.springframework.ui.Model) &&
						!(parametro instanceof org.springframework.validation.BindingResult) &&
						!(parametro instanceof org.springframework.web.servlet.mvc.support.RedirectAttributes)) {
					logger.debug("\t{}: {}", nomeParametro, parametro);
				}
			}
		}
	}
	...
	private String getViewName(Object result) {
		...
		} else if (result instanceof FragmentsRendering) {
			nomeDaView = "Retornando mais de um fragmento via FragmentsRendering";
		}
